...
class FileManager:
        ...
        try:
            for entry in folder_path.iterdir():
                ...
                if entry.suffix.lower() != '.pdf':
                    continue
                # ข้ามไฟล์ที่อยู่ในโฟลเดอร์ผลลัพธ์ (กรณีอยู่ระดับเดียวกัน)
    ...
    def create_pdf_from_form_data(self, pdf_data: Dict, iptnumber_text: str, source_folder: str = None, folder_settings: Dict = None) -> Optional[str]:
        ...
        try:
            ...
            # ถ้า caller ไม่ได้ส่ง folder_settings มา ให้โหลดภายในฟังก์ชัน
            folder_settings = folder_settings or self.read_folder_settings()
            ...
            document_has_vat = self._determine_document_vat_status(pdf_data, folder_settings)
            
            print(f"🏢 ชื่อบริษัท: '{company_name}'")
            print(f"🏷️ ชื่อบริการ: '{service_name}'")
            print(f"🏷️ ประเภทภาษี: '{company_vat_status}'")
            print(f"🏷️ เอกสารมี VAT หรือไม่: '{document_has_vat}'")
            ...
            if company_vat_status == 'VAT' and document_has_vat:
                ...
                new_filename = f"{formatted_date}_{safe_document_number}_{service_name}"
                print(f"📝 ชื่อไฟล์ใหม่ (บริษัท VAT + เอกสาร VAT): '{new_filename}.pdf'")
            else:
                # NoneVat: ไม่ใส่วันที่ในชื่อไฟล์
                new_filename = f"{safe_document_number}_{service_name}"
                print(f"📝 ชื่อไฟล์ใหม่ (ไม่มีวันที่): '{new_filename}.pdf'")
            
            # ลบคำว่า "พ้นกำหนดชำระ" และ "รอชำระ" ออกจากชื่อไฟล์
            ...
            # แทนที่ช่องว่างด้วยขีดล่างเพื่อความสม่ำเสมอ
    ...
    def _determine_document_vat_status(self, pdf_data: Dict, folder_settings: Optional[Dict]) -> bool:
        """ระบุว่าเอกสารถูกจัดเป็น VAT หรือไม่ โดยอิง folder_settings เป็นหลัก"""
        try:
            folder_code = pdf_data.get('folder_code')
            folder_group = None

            if folder_settings and folder_code:
                folder_entry = folder_settings.get(folder_code)
                if isinstance(folder_entry, dict):
                    folder_group = folder_entry.get('group')

            if isinstance(folder_group, str):
                group_normalized = folder_group.strip().lower()
                if group_normalized == 'regular':
                    return True
                if group_normalized == 'special':
                    return False

            # fallback: ดูจาก vat_value ถ้ามีตัวเลขมากกว่า 0 ถือว่าเป็น VAT
            vat_value = pdf_data.get('vat_value')
            if vat_value is not None:
                try:
                    if isinstance(vat_value, str):
                        vat_value_clean = vat_value.replace(',', '').strip()
                        vat_numeric = float(vat_value_clean) if vat_value_clean else 0.0
                    else:
                        vat_numeric = float(vat_value)
                    return vat_numeric > 0
                except Exception:
                    pass

            # ค่า default: ไม่ใส่วันที่ (ถือว่า NoneVat) เพื่อลดโอกาสผิดกติกา
            return False
        except Exception as e:
            print(f"⚠️ เกิดข้อผิดพลาดในการประเมิน VAT ของเอกสาร: {e}")
            return False
