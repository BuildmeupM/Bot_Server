...
try:
    # ลองใช้ __file__ ก่อน
    script_path = Path(__file__)
    if script_path.exists():
        current_dir = script_path.parent.resolve()
    else:
        # ถ้า __file__ ไม่มี ให้ใช้ absolute path
        current_dir = Path(os.path.abspath(__file__)).parent
except Exception:
    # ถ้า resolve ไม่ได้ ให้ใช้ current working directory
    try:
    ...
    try:
        ...
        try:
            ...
            if not isinstance(loop, asyncio.ProactorEventLoop):
                # ถ้าไม่ใช่ Proactor ให้ปิด loop เก่าและสร้างใหม่
                loop.close()
            ...
            # ถ้ายังไม่มี event loop ให้สร้างใหม่ด้วย Proactor
            asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())
...
try:
    ...
    Script directory: {Path(__file__).parent.resolve()}
    Python path: {sys.path[:3]}
    """
    print(error_msg)
    traceback.print_exc()
    # สร้าง stub classes เพื่อให้ Streamlit ยังรันได้ (แต่จะแสดง error)
    class Config:
        BASE_FOLDER = "V"
        MAIN_FOLDERS = []
        SKIP_FOLDERS = []
...
# เก็บข้อมูลโฟลเดอร์ที่กำลังถูกประมวลผลอยู่ (key: normalized folder path, value: job_id)
...
def _normalize_folder_path(folder_path: str) -> str:
    ...
    try:
        ...
        # แปลงเป็น absolute path และ normalize
        normalized = str(folder.resolve())
        # แปลง backslash เป็น forward slash สำหรับความสอดคล้อง
        normalized = normalized.replace('\\', '/')
        return normalized.lower()
    except Exception:
        # ถ้า resolve ไม่ได้ ให้ใช้ path เดิม
...
def _lock_folder(folder_path: str, job_id: str) -> bool:
    ...
    Returns:
        True ถ้า lock สำเร็จ, False ถ้าโฟลเดอร์ถูก lock อยู่แล้ว
    """
    normalized_path = _normalize_folder_path(folder_path)
    with job_store_lock:
        if normalized_path in folder_locks:
            ...
            if existing_job and existing_job.get('status') in ('pending', 'running'):
                ...
                # ถ้า job เสร็จแล้ว ให้ลบ lock เก่า
                folder_locks.pop(normalized_path, None)
...
def _unlock_folder(folder_path: str, job_id: str):
    ...
    normalized_path = _normalize_folder_path(folder_path)
    with job_store_lock:
        # ตรวจสอบว่า lock นี้เป็นของ job นี้จริงๆ
...
def _start_job(folder_path: str) -> str:
    ...
    # ตรวจสอบว่าโฟลเดอร์นี้กำลังถูกประมวลผลอยู่หรือไม่
...
def init_session_state():
    ...
    if 'line_notify_enabled' not in st.session_state:
        # ใช้ getattr เพื่อป้องกันกรณีที่ Config ไม่มี attribute นี้
...
def scan_folders():
    """สแกนโฟลเดอร์ทั้งหมด"""
    try:
        # ใช้ getattr เพื่อป้องกันกรณีที่ Config ไม่มี attribute
...
def add_log(message: str, level: str = "info"):
    ...
    # จำกัดจำนวน log (เก็บแค่ 1000 รายการล่าสุด)
...
def display_logs():
    ...
    for log in st.session_state.logs[-100:]:  # แสดงแค่ 100 รายการล่าสุด
...
def show_txt_editor(folder_code: str):
    ...
    # หาไฟล์ .txt ที่ตรงกับรหัสโฟลเดอร์
...
def show_tax_editor(folder_code: str):
    ...
    # ใช้ form สำหรับเลือกประเภทภาษี
...
def main():
    ...
    # Auto-refresh UI เมื่อมีงานที่ยังทำไม่เสร็จ
