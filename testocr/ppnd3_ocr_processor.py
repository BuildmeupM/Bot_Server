"""
OCR Processor สำหรับแบบภาษี ภ.ง.ด.3 โดยเฉพาะ
...
def get_ppnd3_custom_fields() -> list:
    """
    สร้าง custom fields สำหรับ AksonOCR key-extract API สำหรับแบบภาษี ภ.ง.ด.3
    ...
    return [
        {
            "key": "ภ.ง.ด.3",
            "description": "ค้นหาคำว่า ภ.ง.ด.3 หรือ ภ.ง.ด. 3 ว่ามีไหม",
            "example": "ภ.ง.ด.3"
        },
        {
            "key": "ชื่อบริษัท",
...
def process_ppnd3_ocr(file_path: str, api_key: Optional[str] = None) -> Dict[str, Any]:
    """
    ประมวลผล OCR สำหรับแบบภาษี ภ.ง.ด.3 โดยใช้ AksonOCR API
    ...
    custom_fields = get_ppnd3_custom_fields()
    
    # 3. Prepare payload
    payload = {
        'customFields': json.dumps(custom_fields),
        'model': 'aksonocr-1.0',
        'additionalInstructions': 'ดึงข้อมูลจากแบบภาษี ภ.ง.ด.3 โดยเน้นความถูกต้องของตัวเลขและวันที่'
    }
    ...
    try:
        logger.info(f"📤 [PPND3 OCR] กำลังส่งไฟล์ไปยัง AksonOCR API: {file_path_obj.name}")
        ...
        with open(file_path_obj, 'rb') as file:
            ...
            logger.info(f"📥 [PPND3 OCR] Status Code: {response.status_code}")
            
            if response.status_code in [200, 201]:
                result = response.json()
                logger.info("✅ [PPND3 OCR] สำเร็จ!")
                ...
                if 'success' in result and result.get('success'):
                    ...
                    if isinstance(data, dict):
                        ...
                        if any(key.startswith('1.') or key.startswith('2.') or 'ชื่อบริษัท' in key or 'ภ.ง.ด.3' in key for key in data.keys()):
                            fields_data = data
                ...
                logger.error(f"❌ [PPND3 OCR] Error: {error_msg}")
        ...
        logger.error(f"❌ [PPND3 OCR] {error_msg}")
        return {
            'success': False,
            'data': {},
            'raw_response': {},
        ...
        logger.error(f"❌ [PPND3 OCR] {error_msg}", exc_info=True)
        return {
            'success': False,
            'data': {},
            'raw_response': {},
...
def parse_ppnd3_data(ocr_result: Dict[str, Any]) -> Dict[str, Any]:
    """
    Parse ข้อมูลจาก OCR result ให้เป็นรูปแบบที่ใช้งานได้
    
    Args:
        ocr_result: ผลลัพธ์จาก process_ppnd3_ocr()
    ...
    # process_ppnd3_ocr จะส่ง fields_data โดยตรง (ไม่ wrap อีกชั้น)
    ...
    key_mapping = {
        '1. รวมยอดเงินได้ทั้งสิ้น': 'รวมยอดเงินได้ทั้งสิ้น (ภ.ง.ด.3)',
        '2. รวมยอดภาษีที่นำส่งทั้งสิ้น': 'รวมยอดภาษีที่นำส่งทั้งสิ้น (ภ.ง.ด.3)',
        '3. เงินเพิ่ม(ถ้ามี)': 'เงินเพิ่ม (ภ.ง.ด.3)',
        '4. รวมยอดภาษีที่นำส่งทั้งสิ้น และเงินเพิ่ม (2.+3.)': 'รวมยอดภาษีที่นำส่งทั้งสิ้น และเงินเพิ่ม (ภ.ง.ด.3)',
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python ppnd3_ocr_processor.py <pdf_file_path>")
        sys.exit(1)
    
    file_path = sys.argv[1]
    
    print("=" * 70)
    print("🧪 ทดสอบ PPND3 OCR Processor")
    print("=" * 70)
    print(f"📄 ไฟล์: {file_path}")
    print("-" * 70)
    
    # Process OCR
    result = process_ppnd3_ocr(file_path)
    ...
    if result['success']:
        ...
        # Parse data
        parsed = parse_ppnd3_data(result)
